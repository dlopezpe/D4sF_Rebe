...
from .procesadoImagenesMOISTURE import moisture_img_t2, moisture_img_t1
import asyncio
...
class ImagenProViewSet(generics.ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if capa == 'NDVI':
            ...
            t = Thread(target = moisture_img_t1, kwargs=dict(serializer=serializer, email_user = email, name_user = name))
            #t.daemon = True
            t.start()
...
class ImagenProViewSetTypeTwo(generics.ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if capa == 'NDVI':
            ...
            t = Thread(target = moisture_img_t2, kwargs=dict(serializer=serializer, email_user = email, name_user = name))
            #t.daemon = True
            t.start()
        return Response({"data" : "OK"})
...
class ImagenProViewSetUnificar(generics.ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.save(parcela=parcela, urlSentinel=protocol_url_const, imagen=nombreJson, xlsxFile=nombreXlsx, finifin=finicioFin, tipo=tipo, polygon=[], fechas=datetime.now().strftime('%Y-%m-%d - %H:%M:%S'), capa=capa, fechaElegida=fechaElegida, checkDellOtros=checkDellOtros, alias=alias):
            colorPrint('Creado')
            if(checkDellOtros):
