...
from sentinelhub import WcsRequest, MimeType, CRS, BBox, DataCollection, CustomUrlParam, Geometry, WebFeatureService
...
def ndviImgT2(serializer, email_user, name_user, uid):
    def supProcess(serializer, email_user, name_user, uid):
        ...
        for id_parcela in id_parcelas:
            ...
            cadenaParcelas += str(parcel) + ' -> ' + nombreParcela + '\n'
        fileErr.write(str(cadenaParcelas)+'\n')
        
        fileErr.close()

        #trazaErrores(get_linenumber(), "Hola")
        ...
        try:
            ...
            fileErr.write(" -- Linea 169 - {Error}  " + str(Error) + "\n")
            fileErr.close()
        try:
            ...
            for id_parcela in id_parcelas:
                ...
                try:
                    ...
                    fileErr = open(settings.PARCEL_FOLDER +"catch_"+nombreArchivoErrores, 'a')
                    fileErr.write(id_parcela+" -- Linea 252 - {Error}  " + str(Error) + "\n")
                    fileErr.close()
                ...
                fileErr.write("NDVI URLs = "+str(wms_ndvi_request.get_url_list())+"\n")
                fileErr.close()
                ...
                if includeClouds:
                    ...
                    try:
                        ...
                        fileErr = open(settings.PARCEL_FOLDER +"catch_"+nombreArchivoErrores, 'a')
                        fileErr.write(id_parcela+" -- Linea 302 - {Error} " + str(Error) + "\n")
                        fileErr.close()
                        fallidas+=str(id_parcela)+" "
                        errores=True
                        pass
                    ...
                    fileErr.write("time="+str(time_interval)+"\n")
                    fileErr.write("geometry_string="+str(geometry_string)+"\n")
                    fileErr.write("NDVI request="+str(wms_true_color_clouds_request)+"\n")
                    fileErr.write("NDVI URLs = "+str(wms_true_color_clouds_request.get_url_list())+"\n")
                    fileErr.close()
                    try:
                        ...
                        fileErr = open(settings.PARCEL_FOLDER +"catch_"+ nombreArchivoErrores, 'a')
                        fileErr.write("-- linea 333: fallo al descargar las imagenes TRUE COLOR NUBES de la parcela: "+id_parcela+" " + str(Error) + "\n")
                        fileErr.close()
                        fallidas+=str(id_parcela)+" "
                        errores=True
                        pass
                    ...
                    try:
                        ...
                        fileErr = open(settings.PARCEL_FOLDER + "catch_"+nombreArchivoErrores, 'a')
                        fileErr.write(id_parcela+" -- Linea 364 - {Error} " + str(Error) + "\n")
                        fileErr.close()
                        fallidas+=str(id_parcela)+" "
                        errores=True
                        pass
                ...
                try:
                    ...
                    fileErr.write("NDVI URLs = "+str(wms_true_color_request.get_url_list())+"\n")
                    fileErr.close()
                except Exception as Error:
                    fileErr = open(settings.PARCEL_FOLDER +"catch_"+ nombreArchivoErrores, 'a')
                    fileErr.write(id_parcela+" -- Linea 400 - {Error} " + str(Error) + " \n")
                    fileErr.close()
                    fallidas+=str(id_parcela)+" "
                    errores=True
                    pass
                    ...
                    if str(time.date().isoformat()) != fechaAnterior:
                        ...
                        try:
                            ...
                            if includeClouds:
                                ...
                                pixeles_nubes = NpEncoder.get_pixeles(np.asarray((unique, counts)).T)
                            ...
                            total_pixeles_amarillo = {"total": pixeles_amarillos,
                                                    ...
                                                    "altos": {"total": pixeles_amarillos_altos,
                                                                ...
                                                                "area_porcent": (
                                                                        NpEncoder.percentage(pixeles_amarillos_altos,
                                                                                total_pixeles) * area / 100)},
                                                    "medios": {"total": pixeles_amarillos_medios,
                                                                "porcent": NpEncoder.percentage(pixeles_amarillos_medios, total_pixeles),
                                                                "area_porcent": (
                                                    ...
                                                    "bajos": {"total": pixeles_amarillos_bajos,
                                                                ...
                                                                "area_porcent": (
                                                                        NpEncoder.percentage(pixeles_amarillos_bajos,
                                                                                total_pixeles) * area / 100)}}
                            fileErr = open(settings.PARCEL_FOLDER + nombreArchivoErrores, 'a')
                            fileErr.write("UnionesT------Amarillos\n")
                            fileErr.close()
                            total_pixeles_rojo = {"total": pixeles_rojos, "porcent": NpEncoder.percentage(pixeles_rojos, arr_total[1][1]),
                            ...
                            fileErr.close()
                            total_pixeles_verde = {"total": pixeles_verdes, "porcent": NpEncoder.percentage(pixeles_verdes, arr_total[1][1]),
                            ...
                            if includeClouds:
                                ...
                                total_pixeles_nubes = {"total": "yes", "porcent": str(results_cloud_wfs_invertido[idx]['properties']['cloudCoverNpEncoder.percentage'])}
                            else:
                            ...
                            print(Error)
                            fileErr = open(settings.PARCEL_FOLDER +"catch_"+ nombreArchivoErrores, 'a')
                            fileErr.write(id_parcela+" -- Linea 699 - {Error} " + str(Error) + " \n")
                            fileErr.close()
            ...
            fileErr = open(settings.PARCEL_FOLDER +"catch_"+ nombreArchivoErrores, 'a')
            fileErr.write(id_parcela+" -- Linea 958 - {Error} " + str(Error) + " \n")
            fileErr.close()
