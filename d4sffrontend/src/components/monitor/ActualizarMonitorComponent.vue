            ...
            <div class="bx--col-sm-12 bx--col-md-2 bx--col-lg-2">
                ...
                >
                    ...
                    <cv-select-optgroup :label="`Cooperativa ${cooperativa.name}`" v-bind:key="cooperativa.id" v-for="cooperativa in arrCooperativas">
                        ...
                        <cv-select-option v-for="empresa in getSortedCooperativa(cooperativa).enterprises" 
                            :value="String(empresa.id)"  
                            v-bind:key="empresa.id">
                            <!--si la empresa tiene valor is_monitor a true -->
                            {{empresa.name }}
                        </cv-select-option>
                    </cv-select-optgroup>
                </cv-select>
...
import {getAllActiveEnterprisesAndMonitor, getEnterprise} from '@/crudFunctions/crudEnterprise'//getFechasSentinelForInformes
...
export default {
    ...
    methods: {
        ...
        seleccionaEmpresa(empresa){
            const arrParcels = []
            if(empresa == 'all'){
                getAllActiveEnterprisesAndMonitor()
                    .then(async response => {
