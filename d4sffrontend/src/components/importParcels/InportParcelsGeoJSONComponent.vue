<template>
    <div class="bx--grid">
        ...
        <div id="txt-explicativo" class="bx--row">
            <div class="bx--col">
                <h2>Importar Parcelas desde GeoJSON <img src="../../assets/agriculture-analytics.svg" style="width: 40px;"></h2>
            </div>
        </div>
        <br>
        <div id="txt-explicativo" class="bx--row">
            <div class="bx--col">
                <p>Para poder importar parcelas desde un .zip, este tiene que contener archivos con las extensiones xxx.geojson</p>
            </div>
        </div>
        <br>
        <div class="bx--row" style="margin-top: 20px;">
            ...
            <!--TOGGLE PARA BORRAR O AÑADIR LAS PARCELAS-->
            <div class="bx--col">
                <cv-select
                    theme=""
                    ...
                    :inline="false"
                    :disabled="false"
                    @change="actionChangedToggleModo"
                >
                    <cv-select-option value="true">Eliminar</cv-select-option>
                    <cv-select-option value="false">Añadir</cv-select-option>
                </cv-select>
            </div>
...
import {importParcelsFromGeoJSON} from '@/crudFunctions/crudEnterprise'
...
export default {
    ...
    methods: {
        ...
        importarParcelas(){
            ...
            if(this.eliminar==""){
                console.info('No hay acción')
                this.mensajeAlerta = `Debe seleccionar una acción`
                this.tipoAlerta = `error`
                this.$refs.alertaGenerall.verAlerta()
                return
            }
            ...
            importParcelsFromGeoJSON(this.file[0].file, this.selectValueEmpresa, false,this.eliminar)
            .then(response => {
        ...
        importarParcelasConfirmar(){
            ...
            console.info('Importando parcelas')
            this.isActiveLoading = true
            importParcelsFromGeoJSON(this.file[0].file, this.selectValueEmpresa, true,this.eliminar)
            .then(() => {
                this.isActiveLoading = false
