...
from .procesar_MOISTURE_INDEX import procesarMOISTURE
from .procesar_fechas import ndviImgFechas, getBBOX
import pandas as pd
import uuid
from .jsontoexcel import json2xlsx

from trazas.utils import savelog,getRequestIp
...
class NewImagenProViewSet(generics.ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if capa == 'NDVI':
            ...
            t = Thread(target=procesarMOISTURE, kwargs=dict(serializer=serializer, email_user=email, name_user=name, uid=uid,request=self.request))
            t.start()
...
class NewImagenProFechasViewSet(generics.ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        id_parcelas = serializer.validated_data['id_parcelas']
        retorno = ndviImgFechas(serializer)
