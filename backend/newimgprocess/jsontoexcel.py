...
def json2xlsx(jsonFile, xlsxFile, capa, tipo = 0):
    ...
    if capa == 'NDVI':
        ...
        if tipo==1:
            for item in pj:
                worksheet.write_string(pj.columns.size+2+9, col+1, pj[item]['nombre']+' - '+pj[item]['fecha'], header_format)
                worksheet.insert_image(pj.columns.size+2+12, col+1, settings.PARCEL_FOLDER+str(pj[item]['img']), {'x_scale': 0.2, 'y_scale': 0.2})
                if pj[item]['nubes']['porcent'] != "n/a":
                    worksheet.insert_image(pj.columns.size + 2 + 18, col + 1, settings.PARCEL_FOLDER+'cloud_' + str(pj[item]['img']), {'x_scale': 0.1, 'y_scale': 0.1})
                worksheet.insert_image(pj.columns.size + 2 + 24, col + 1, settings.PARCEL_FOLDER+'trueColor_' + str(pj[item]['img']), {'x_scale': 0.2, 'y_scale': 0.2})

                #pag 2
                worksheet2.write_string(pj.columns.size+2+9, col+1, pj[item]['nombre']+' - '+pj[item]['fecha'], header_format)
                worksheet2.insert_image(pj.columns.size+2+12, col+1, settings.PARCEL_FOLDER+str(pj[item]['img']), {'x_scale': 0.2, 'y_scale': 0.2})
                if pj[item]['nubes']['porcent'] != "n/a":
                    worksheet2.insert_image(pj.columns.size + 2 + 18, col + 1, settings.PARCEL_FOLDER+'cloud_' + str(pj[item]['img']), {'x_scale': 0.1, 'y_scale': 0.1})
                worksheet2.insert_image(pj.columns.size + 2 + 24, col + 1, settings.PARCEL_FOLDER+'trueColor_' + str(pj[item]['img']), {'x_scale': 0.2, 'y_scale': 0.2})
