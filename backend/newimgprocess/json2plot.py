...
def plotMoisture(jsonFile, plotImgName):
    df = pd.read_json(settings.PARCEL_FOLDER+jsonFile)
    df = df.transpose()
    ...
    axis = dataframe.plot(kind="barh", color=['#ff8000', '#ffdf00', '#66ff98', '#02fefc', '#0087ff', '#2000ff', '#9901c4'])
    print(axis)
    plt.savefig(settings.PARCEL_FOLDER+plotImgName, bbox_inches='tight')
    plt.close()

def plotNDVI(jsonFile, plotImgName):
    df = pd.read_json(settings.PARCEL_FOLDER + jsonFile)
    df = df.transpose()
